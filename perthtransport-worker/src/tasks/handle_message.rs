...
pub async fn handle_message(
    ...
    let s = message.get_payload::<String>()?;
        ...
        PubSubAction::Hello => {
            ...
            for trip_id in live_trip_ids {
                ...
                let task_created = task_manager
                    .add_task_to_websocket_session(socket_id.clone(), trip_id.clone(), || {
                        let http_client = http_client.clone();
                        let span = tracing::span!(Level::INFO, "trip_task", trip_id = trip_id);
                        ...
                        tokio::spawn(async move {
                            if let Err(e) =
                                handle_trip(http_client, worker_tx, config, trip_id_cloned)
                                    .instrument(span)
                                    .await
                            {
                ...
                if !task_created {
                    ...
                    if let Ok(cache_value) = cache_value {
                        redis_multiplexed_lock
                            .publish(
                                format!("{}_{}", PUBSUB_CHANNEL_OUT_PREFIX, message.socket_id),
                                cache_value,
                            )
                            .await?
                    }
